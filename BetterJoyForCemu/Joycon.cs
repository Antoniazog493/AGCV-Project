...
namespace BetterJoyForCemu {
    public class Joycon {
        ...
        public DebugType debug_type = (DebugType)CachedConfig.DebugType;
        public bool isLeft;
        ...
        private readonly byte[] stick_raw = new byte[3];
        private readonly UInt16[] stick_cal = new UInt16[6];
        private UInt16 deadzone;
        private readonly UInt16[] stick_precal = new UInt16[2];

        private readonly byte[] stick2_raw = new byte[3];
        private readonly UInt16[] stick2_cal = new UInt16[6];
        private UInt16 deadzone2;
        private readonly UInt16[] stick2_precal = new UInt16[2];
        ...
        // Latency monitoring (optional, for debugging)
        private BetterJoyForCemu.Diagnostics.LatencyMonitor _latencyMonitor = null;
        private const int LATENCY_LOG_INTERVAL = 2000; // Log every 2000 packets
        ...
        private MadgwickAHRS AHRS = new MadgwickAHRS(0.005f, CachedConfig.AHRS_Beta); // for getting filtered Euler angles of rotation; 5ms sampling rate
        ...
        public Joycon(IntPtr handle_, bool imu, bool localize, float alpha, bool left, string path, string serialNum, int id = 0, bool isPro = false, bool isSnes = false, bool is64 = false, bool thirdParty = false) {
            ...
            rumble_obj = new RumbleData(CachedConfig.LowFreqRumble, CachedConfig.HighFreqRumble, 0);
            ...
            if (CachedConfig.ShowAsXInput) {
                out_xbox = new OutputControllerXbox360();
                if (CachedConfig.EnableRumble)
                    out_xbox.FeedbackReceived += ReceiveRumble;
            }

            if (CachedConfig.ShowAsDS4) {
                out_ds4 = new OutputControllerDualShock4();
                if (CachedConfig.EnableRumble)
                    out_ds4.FeedbackReceived += Ds4_FeedbackReceived;
            }
            
            // Initialize latency monitor for debugging if enabled
            if (debug_type == DebugType.THREADING || debug_type == DebugType.ALL) {
                _latencyMonitor = new BetterJoyForCemu.Diagnostics.LatencyMonitor();
            }
        }
        ...
        public void ReceiveRumble(Xbox360FeedbackReceivedEventArgs e) {
            ...
            SetRumble(CachedConfig.LowFreqRumble, CachedConfig.HighFreqRumble, (float)Math.Max(e.LargeMotor, e.SmallMotor) / 255f);

            if (other != null && other != this)
                other.SetRumble(CachedConfig.LowFreqRumble, CachedConfig.HighFreqRumble, (float)Math.Max(e.LargeMotor, e.SmallMotor) / 255f);
        }
        ...
        public void Detach(bool close = false) {
            ...
            // Clean up pair synchronizer
            string pairKey = GetPairKey();
            if (pairKey != null) {
                _pairSynchronizers.TryRemove(pairKey, out _);
            }
            _pairSync = null;
        ...
        private byte ts_en;
        // Reuse buffer to reduce GC pressure (hot path optimization)
        private readonly byte[] _rawBufReusable = new byte[report_len];
        
        private int ReceiveRaw() {
            if (handle == IntPtr.Zero) return -2;
            // Reuse buffer instead of allocating each time
            byte[] raw_buf = _rawBufReusable;
            // Reduced timeout from 1ms to non-blocking for faster polling
            // Note: This requires more CPU but reduces latency significantly
            int ret = HIDapi.hid_read_timeout(handle, raw_buf, new UIntPtr(report_len), 0);
            ...
            if (ret > 0) {
                ...
                for (int n = 0; n < 3; n++) {
                    ...
                    // Track latency if monitoring enabled
                    _latencyMonitor?.RecordUpdate(isLeft);
                    
                    // Log detailed stats periodically
                    if (_latencyMonitor != null && packetCounter % LATENCY_LOG_INTERVAL == 0) {
                        DebugPrint(_latencyMonitor.GetDetailedStats(), DebugType.THREADING);
                    }
        ...
        // Removed: ChangeOrientationDoubleClick feature - was causing accidental misclicks
        ...
        private void DoThingsWithButtons() {
            ...
            if (CachedConfig.HomeLongPowerOff && buttons[powerOffButton]) {
            ...
            // REMOVED: Double-click L3/R3 to change orientation feature
            // This was causing accidental misclicks

            if (CachedConfig.PowerOffInactivityMins > 0) {
                if ((timestamp - inactivity) / 10000 > CachedConfig.PowerOffInactivityMins * 60 * 1000) {
                    if (other != null)
                        other.PowerOff();
            ...
            // Only simulate Home button if it's custom-mapped (not default "0")
            // This prevents triggering MS GameBar or other system shortcuts
            string homeMapping = Config.Value("home");
            if (buttons_down[(int)Button.HOME] && homeMapping != "0")
                Simulate(homeMapping);

            SimulateContinous((int)Button.CAPTURE, Config.Value("capture"));

            // Only simulate continuous Home if custom-mapped
            if (homeMapping != "0")
            ...
            if (CachedConfig.GyroAnalogSliders && (other != null || isPro)) {
                ...
                int ldy, rdy;
                if (CachedConfig.UseFilteredIMU) {
                    ldy = (int)(CachedConfig.GyroAnalogSensitivity * (left.cur_rotation[0] - left.cur_rotation[3]));
                    rdy = (int)(CachedConfig.GyroAnalogSensitivity * (right.cur_rotation[0] - right.cur_rotation[3]));
                } else {
                    ldy = (int)(CachedConfig.GyroAnalogSensitivity * (left.gyr_g.Y * dt));
                    rdy = (int)(CachedConfig.GyroAnalogSensitivity * (right.gyr_g.Y * dt));
                }
            ...
            if (res_val.StartsWith("joy_")) {
                ...
                if (CachedConfig.GyroHoldToggle) {
            ...
            if (CachedConfig.GyroToJoyOrMouse.Substring(0, 3) == "joy") {
                if (Config.Value("active_gyro") == "0" || active_gyro) {
                    float[] control_stick = (CachedConfig.GyroToJoyOrMouse == "joy_left") ? stick : stick2;

                    float dx, dy;
                    if (CachedConfig.UseFilteredIMU) {
                        dx = (CachedConfig.GyroStickSensitivityX * (cur_rotation[1] - cur_rotation[4])); // yaw
                        dy = -(CachedConfig.GyroStickSensitivityY * (cur_rotation[0] - cur_rotation[3])); // pitch
                    } else {
                        dx = (CachedConfig.GyroStickSensitivityX * (gyr_g.Z * dt)); // yaw
                        dy = -(CachedConfig.GyroStickSensitivityY * (gyr_g.Y * dt)); // pitch
                    }

                    control_stick[0] = Math.Max(-1.0f, Math.Min(1.0f, control_stick[0] / CachedConfig.GyroStickReduction + dx));
                    control_stick[1] = Math.Max(-1.0f, Math.Min(1.0f, control_stick[1] / CachedConfig.GyroStickReduction + dy));
                }
            } else if (CachedConfig.GyroToJoyOrMouse == "mouse" && (isPro || (other == null) || (other != null && (Boolean.Parse(ConfigurationManager.AppSettings["GyroMouseLeftHanded"]) ? isLeft : !isLeft)))) {
                // gyro data is in degrees/s
                if (Config.Value("active_gyro") == "0" || active_gyro) {
                    int dx, dy;

                    if (CachedConfig.UseFilteredIMU) {
                        dx = (int)(CachedConfig.GyroMouseSensitivityX * (cur_rotation[1] - cur_rotation[4])); // yaw
                        dy = (int)-(CachedConfig.GyroMouseSensitivityY * (cur_rotation[0] - cur_rotation[3])); // pitch
                    } else {
                        dx = (int)(CachedConfig.GyroMouseSensitivityX * (gyr_g.Z * dt));
                        dy = (int)-(CachedConfig.GyroMouseSensitivityY * (gyr_g.Y * dt));
                    }
        ...
        private void Poll() {
            ...
            while (!stop_polling & state > state_.NO_JOYCONS) {
                ...
                if (a > 0 && state > state_.DROPPED) {
                    ...
                    attempts = 0;
                    spinCount = 0; // Reset spin count on successful read
                    ...
                    // An error on read - use SpinWait for low-latency retry
                    if (spinCount < 100) {
                        Thread.SpinWait(20); // Spin for ~1-2μs instead of yielding
                        spinCount++;
                    } else {
                        Thread.Sleep(0); // Yield after many spins to prevent CPU throttle
                        spinCount = 0;
                    }
                    ++attempts;
                } else if (a == 0) {
                    // Non-blocking read returned no data - spin briefly before retry
                    if (spinCount < 50) {
                        Thread.SpinWait(10);
                        spinCount++;
                    } else {
        ...
        // Synchronizer for paired Joy-Cons to reduce latency asymmetry
        private static readonly System.Collections.Concurrent.ConcurrentDictionary<string, JoyconPairSynchronizer> 
            _pairSynchronizers = new System.Collections.Concurrent.ConcurrentDictionary<string, JoyconPairSynchronizer>();
        private JoyconPairSynchronizer _pairSync = null;
        
        private string GetPairKey() {
            if (other == null || other == this) return null;
            // Create consistent key regardless of which Joy-Con creates it
            var leftSerial = isLeft ? serial_number : other.serial_number;
            var rightSerial = isLeft ? other.serial_number : serial_number;
            return $"{leftSerial}_{rightSerial}";
        }
        ...
        private int ProcessButtonsAndStick(byte[] report_buf) {
            ...
            if (!isSnes) {
                ...
                stick = CenterSticks(stick_precal, stick_cal, deadzone, isLeft ? CachedConfig.StickScalingFactor : CachedConfig.StickScalingFactor2);

                if (isPro) {
                    stick2_precal[0] = (UInt16)(stick2_raw[0] | ((stick2_raw[1] & 0xf) << 8));
                    stick2_precal[1] = (UInt16)((stick2_raw[1] >> 4) | (stick2_raw[2] << 4));
                    stick2 = CenterSticks(stick2_precal, stick2_cal, deadzone2, CachedConfig.StickScalingFactor2);
                }

                // Read other Joycon's sticks with optimized synchronization and prediction
                if (other != null && other != this) {
                    // Get or create pair synchronizer
                    string pairKey = GetPairKey();
                    if (pairKey != null && _pairSync == null) {
                        _pairSync = _pairSynchronizers.GetOrAdd(pairKey, _ => new JoyconPairSynchronizer());
                    }
                    
                    if (isLeft) {
                        // LEFT Joy-Con: Use PREDICTION to compensate for latency
                        _pairSync?.UpdateLeftStick(stick);
                        // Get predicted right stick position (helps with lag)
                        stick2 = _pairSync?.GetRightStick() ?? otherStick;
                    } else {
                        // RIGHT Joy-Con: Update normally
                        _pairSync?.UpdateRightStick(stick);
                        // Use PREDICTED left stick to compensate for its higher latency
                        var leftStick = _pairSync?.GetLeftStickPredicted() ?? otherStick;
                        Array.Copy(stick, stick2, 2);
                        stick = leftStick;
                    }
                } else if (isLeft && other != null && other != this) {
                    // Fallback to old method if sync not available
                    stick2 = otherStick;
            ...
            // Optimized: Use single lock instead of nested locks
            lock (buttons) {
                // Cache previous state without additional lock
                bool[] previousState = down_;
                down_ = buttons;
                // Clear current buttons array instead of allocating new
                ...
                // Process button changes without nested locks
                bool changed = false;
                for (int i = 0; i < buttons.Length; ++i) {
                    buttons_up[i] = (previousState[i] & !buttons[i]);
                    buttons_down[i] = (!previousState[i] & buttons[i]);
                    if (previousState[i] != buttons[i]) {
                        buttons_down_timestamp[i] = (buttons[i] ? timestamp : -1);
                        changed = true;
                    }
                }

                if (changed) inactivity = timestamp;
            }
        ...
        public void Begin() {
            if (PollThreadObj == null) {
                ...
                // Set high priority to reduce input latency
                // This helps especially with left Joy-Con latency issues
                try {
                    PollThreadObj.Priority = ThreadPriority.AboveNormal;
                } catch (Exception ex) {
                    form.AppendTextBox($"Could not set thread priority: {ex.Message}\r\n");
                }
        ...
        public void SetRumble(float low_freq, float high_freq, float amp) {
            ...
            rumble_obj.SetValues(low_freq, high_freq, amp);
        }
        ...
        private static OutputControllerXbox360InputState MapToXbox360Input(Joycon input) {
            ...
            if (is64) {
                ...
                output.a = buttons[(int)(!CachedConfig.SwapAB ? Button.B : Button.A)];
                output.b = buttons[(int)(!CachedConfig.SwapAB ? Button.A : Button.B)];
                ...
                output.a = buttons[(int)(!CachedConfig.SwapAB ? Button.B : Button.A)];
                output.b = buttons[(int)(!CachedConfig.SwapAB ? Button.A : Button.B)];
                output.y = buttons[(int)(!CachedConfig.SwapXY ? Button.X : Button.Y)];
                output.x = buttons[(int)(!CachedConfig.SwapXY ? Button.Y : Button.X)];
                ...
                if (other != null) { // no need for && other != this
                    output.a = buttons[(int)(!CachedConfig.SwapAB ? isLeft ? Button.B : Button.DPAD_DOWN : isLeft ? Button.A : Button.DPAD_RIGHT)];
                    output.b = buttons[(int)(CachedConfig.SwapAB ? isLeft ? Button.B : Button.DPAD_DOWN : isLeft ? Button.A : Button.DPAD_RIGHT)];
                    output.y = buttons[(int)(!CachedConfig.SwapXY ? isLeft ? Button.X : Button.DPAD_UP : isLeft ? Button.Y : Button.DPAD_LEFT)];
                    output.x = buttons[(int)(CachedConfig.SwapXY ? isLeft ? Button.X : Button.DPAD_UP : isLeft ? Button.Y : Button.DPAD_LEFT)];
                    ...
                    output.y = buttons[(int)(!CachedConfig.SwapXY ? isLeft ? Button.DPAD_RIGHT : Button.DPAD_LEFT : isLeft ? Button.DPAD_UP : Button.DPAD_DOWN)];
                    output.x = buttons[(int)(CachedConfig.SwapXY ? isLeft ? Button.DPAD_RIGHT : Button.DPAD_LEFT : isLeft ? Button.DPAD_UP : Button.DPAD_DOWN)];
            ...
            if (!is64) {
                if (other != null || isPro) {
                    byte lval = CachedConfig.GyroAnalogSliders ? sliderVal[0] : Byte.MaxValue;
                    byte rval = CachedConfig.GyroAnalogSliders ? sliderVal[1] : Byte.MaxValue;
        ...
        public static OutputControllerDualShock4InputState MapToDualShock4Input(Joycon input) {
            ...
            if (is64) {
                ...
                output.cross = buttons[(int)(!CachedConfig.SwapAB ? Button.B : Button.A)];
                output.circle = buttons[(int)(!CachedConfig.SwapAB ? Button.A : Button.B)];
            ...
            if (isPro) {
                output.cross = buttons[(int)(!CachedConfig.SwapAB ? Button.B : Button.A)];
                output.circle = buttons[(int)(!CachedConfig.SwapAB ? Button.A : Button.B)];
                output.triangle = buttons[(int)(!CachedConfig.SwapXY ? Button.X : Button.Y)];
                output.square = buttons[(int)(!CachedConfig.SwapXY ? Button.Y : Button.X)];
                ...
                output.options = buttons[(int)Button.PLUS];
                // Don't map Home to PS unless explicitly configured (prevents MS GameBar)
                output.ps = Config.Value("home") != "0" && buttons[(int)Button.HOME];
                ...
                if (other != null) { // no need for && other != this
                    output.cross = !CachedConfig.SwapAB ? buttons[(int)(isLeft ? Button.B : Button.DPAD_DOWN)] : buttons[(int)(isLeft ? Button.A : Button.DPAD_RIGHT)];
                    output.circle = CachedConfig.SwapAB ? buttons[(int)(isLeft ? Button.B : Button.DPAD_DOWN)] : buttons[(int)(isLeft ? Button.A : Button.DPAD_RIGHT)];
                    output.triangle = !CachedConfig.SwapXY ? buttons[(int)(isLeft ? Button.X : Button.DPAD_UP)] : buttons[(int)(isLeft ? Button.Y : Button.DPAD_LEFT)];
                    output.square = CachedConfig.SwapXY ? buttons[(int)(isLeft ? Button.X : Button.DPAD_UP)] : buttons[(int)(isLeft ? Button.Y : Button.DPAD_LEFT)];
                    ...
                    output.cross = !CachedConfig.SwapAB ? buttons[(int)(isLeft ? Button.DPAD_LEFT : Button.DPAD_RIGHT)] : buttons[(int)(isLeft ? Button.DPAD_DOWN : Button.DPAD_UP)];
                    output.circle = CachedConfig.SwapAB ? buttons[(int)(isLeft ? Button.DPAD_LEFT : Button.DPAD_RIGHT)] : buttons[(int)(isLeft ? Button.DPAD_DOWN : Button.DPAD_UP)];
                    output.triangle = !CachedConfig.SwapXY ? buttons[(int)(isLeft ? Button.DPAD_RIGHT : Button.DPAD_LEFT)] : buttons[(int)(isLeft ? Button.DPAD_UP : Button.DPAD_DOWN)];
                    output.square = CachedConfig.SwapXY ? buttons[(int)(isLeft ? Button.DPAD_RIGHT : Button.DPAD_LEFT)] : buttons[(int)(isLeft ? Button.DPAD_UP : Button.DPAD_DOWN)];
